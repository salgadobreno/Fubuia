module EventCalendar
  module CalendarHelper
    ...
    def calendar(options = {}, &block)
      ...
      row_height = cal_row_heights(options)
      height = options[:day_names_height] + row_height
      ...
      first = data_atual - 3 + options[:shift].to_i
      last = data_atual + 3 + options[:shift].to_i
      ...
      #se shift for negativo => data_atual.wday + 6
      if options[:shift].to_i >= 0
        (data_atual.wday + 4 + options[:shift].to_i).times do
          day_names.push(day_names.shift)
        end
      else
        (10 - data_atual.wday + options[:shift].abs.to_i).times do
      ...
      # day names
      cal << %(<table class="ec-day-names mediaTable" style="height: #{options[:day_names_height]}px;" cellpadding="0" cellspacing="0">)
      cal << %(<tbody><tr>)
      day_names.each_with_index do |day_name, index|
        cal << %(<th class="ec-day-name optional" title="#{day_name}">#{day_name}</th>) if [1,5].include?(index)

        cal << %(<th class="ec-day-name essential" title="#{day_name}">#{day_name}</th>) if [2,3,4].include?(index)

        cal << %(<th class="ec-day-name" title="#{day_name}">#{day_name}</th>) if [0,6].include?(index)
      end
      cal << %(</tr></tbody></table>)
      ...
      last_day_of_week = last
      row_num = 0
      top = 0

      # go through a week at a time, until we reach the end of the month
      #BRENO: last day of calendar removed because there's no loop anymore
      #while(last_day_of_week <= last_day_of_cal)
      cal << %(<div class="ec-row" style="top: #{top}px; height: #{row_height}px;">)
      #top += row_heights[row_num]

      # this weeks background table
      cal << %(<table class="ec-row-bg mediaTable" cellpadding="0" cellspacing="0">)
      cal << %(<tbody><tr>)
      ...
      first_day_of_week.upto(first_day_of_week+6) do |day|
        ...
        cal << %(<td class="ec-day-bg #{today_class} #{other_month_class}">&nbsp;</td>) if [0,6].include?(yndex)
        cal << %(<td class="ec-day-bg essential #{today_class} #{other_month_class}">&nbsp;</td>) if [2,3,4].include?(yndex)
        cal << %(<td class="ec-day-bg optional #{today_class} #{other_month_class}">&nbsp;</td>) if [1,5].include?(yndex)
      end
      cal << %(</tr></tbody></table>)

      # calendar row
      cal << %(<table class="ec-row-table mediaTable" cellpadding="0" cellspacing="0">)
      cal << %(<tbody>)

      # day numbers row
      cal << %(<tr>)

      #BRENO: ugly, getting the index to implement mediatable
      index = 0

      first_day_of_week.upto(last_day_of_week) do |day|
        cal << %(<td class="ec-day-header optional ) if [1,5].include?(index)
        cal << %(<td class="ec-day-header essential ) if [2,3,4].include?(index)
        cal << %(<td class="ec-day-header ) if [0,6].include?(index)
        index = index + 1 #//ugly

        cal << %(ec-today-header ) if options[:show_today] and (day == Date.today)
        #cal << %(ec-other-month-header ) if (day < first) || (day > last)
        cal << %(ec-other-month-header) if (day.month < options[:month]) || (day.month > options[:month])
        cal << %(ec-tuesday-header) if tuesday?(day)
        cal << %(ec-friday-header) if friday?(day)
